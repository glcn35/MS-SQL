...
SELECT city, COUNT(city) as city_count FROM sale.customer 
WHERE state='TX'
GROUP BY city

---- 2. List all the cities in the California which has more than 5 customer, by showing the cities which have more customers first.  Kaliforniya'da 5'ten fazla müþterisi olan tüm þehirleri önce en fazla müþterisi olan þehirleri göstererek listeleyiniz.------
SELECT city,count(city) FROM sale.customer 
WHERE state='CA' 
GROUP BY city 
ORDER by city
...
---- 3. List the top 10 most expensive products----3. En pahalý 10 ürünü listeleyin-
...
from sale.store A 
join product.stock B on A.store_id=B.store_id
JOIN product.product C on C.product_id=B.product_id 
WHERE quantity>25 and B.store_id=2
...
SELECT staff_id, 
	YEAR(shipped_date)AS date_year,
	count(YEAR(shipped_date)) as date_year_count
...
SELECT B.brand_id,B.brand_name, YEAR(D.order_date) AS date_year,SUM(c.quantity) AS sum_quantity
FROM product.product A
JOIN product.brand B
ON A.brand_id=B.brand_id
JOIN sale.order_item C
...
SELECT A.store_name, YEAR(B.order_date) AS date_year,SUM(c.quantity) AS sum_quantity
FROM sale.store A
JOIN sale.orders B
...
SELECT TOP 1 A.store_name, YEAR(B.order_date) AS date_year,SUM(c.quantity) AS sum_quantity
FROM sale.store A
JOIN sale.orders B
ON A.store_id=B.store_id
JOIN sale.order_item C
...
SELECT TOP 1 A.staff_id, A.first_name, A.last_name, YEAR(B.order_date),COUNT(C.quantity)
FROM sale.staff A
JOIN sale.orders B
ON A.staff_id=B.staff_id
JOIN sale.order_item C
